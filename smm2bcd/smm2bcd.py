...
def read_main_attributes():
    ...
    with open(dec_course_path,'rb') as course:
        for i in range(2600):
            ...
            if high == '0x6' or high == '0x7' or high == '0xE' or high == '0xF':
                size = '2.5x Size'
            print(str(i) + ': ' + status + ', ' + size + ', ' + rotation + ' Entity: ' + str(high) + ' ' + str(low) + ' @' + str(hex(628 + 32 * i)))
...
def read_main_entities():
    global dec_course_path
    with open(dec_course_path,'rb') as course:
        for i in range(2600):
            course.seek(624 + 32 * i)
...
def read_secondary_entities():
    global dec_course_path
    with open(dec_course_path,'rb') as course:
        for i in range(2600):
            course.seek(626 + 32 * i)
            entity_id = course.read(3)
            entity_name = f'Unknown({entity_id})'
            if entity_id == bytes([0x00, 0x00, 0xFF]) or entity_id == bytes([0x00, 0x12, 0xFF]):
                entity_name = 'Goomba/Galoomba'
...
def print_entities():
    ...
    id += 1
    print('ID ' + str(id) + ', ' + hex(id) + ': Coin/Frozen Coin')
    id += 1
    ...
    id += 1
    print('ID ' + str(id) + ', ' + hex(id) + ': Super Mushroom/Master Sword')
    id += 1
    ...
    id += 1
    print('ID ' + str(id) + ', ' + hex(id) + ': Spike/Spike Ball')
    id += 1
    ...
    id += 1
    print('ID ' + str(id) + ', ' + hex(id) + ': Pokey')
    id += 1
    print('ID ' + str(id) + ', ' + hex(id) + ': P-Block')
    id += 1
    print('ID ' + str(id) + ', ' + hex(id) + ': Dash Block')
    id += 1
...
def edit_entity(entity_name, entity_number):
    ...
    if entity_name == 'Ground':
        new_id = 0x07
    if entity_name == 'Coin' or entity_name == 'Frozen Coin':
        new_id = 0x08
    if entity_name == 'Pipe':
        new_id = 0x09
    ...
    if entity_name == 'Pow Block':
        new_id = 0x13
    if entity_name == 'Super Mushroom' or entity_name == 'Master Sword':
        new_id = 0x14
    ...
    if entity_name == 'Boom Boom':
        new_id = 0x4D
    if entity_name == 'Pokey':
        new_id = 0x4E
    if entity_name == 'P-Block':
        new_id = 0x4F
    if entity_name == 'Dash Block':
        new_id = 0x50
    if entity_name == 'None':
        new_id = 0x51
    ...
    with open(dec_course_path, 'rb') as course:
	    data = course.read()
	    data = bytearray(data)
	    data[624 + 32 * entity_number] = int(new_id)
...
def edit_key_status(entity_number, key_status):
    ...
    with open(dec_course_path, 'rb') as course:
        data = course.read()
        data = bytearray(data)
        data[625 + 32 * entity_number] = int(key)
        with open(dec_course_path, 'wb') as course:
            course.write(data)
            print('Success!')
...
def remove_entity(entity_number):
    ...
    with open(dec_course_path, 'rb') as course:
        data = course.read()
        data = bytearray(data)
        data[int(600 + 32 * entity_number):int(552 + 32 * entity_number + 32)] = bytes([0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00])
        with open(dec_course_path, 'wb') as course:
            course.write(data)
